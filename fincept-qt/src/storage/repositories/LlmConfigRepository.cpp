...
LlmConfig LlmConfigRepository::map_config(QSqlQuery& q) {
    return {q.value(0).toString(), q.value(1).toString(), q.value(2).toString(), q.value(3).toString(),
            q.value(4).toString(), q.value(5).toBool(),   q.value(6).toBool(),   q.value(7).toString(),
            q.value(8).toString()};
}
...
Result<QVector<LlmConfig>> LlmConfigRepository::list_providers() {
    return query_list("SELECT provider, api_key, base_url, model, reasoning_effort, is_active, tools_enabled, "
...
Result<LlmConfig> LlmConfigRepository::get_active_provider() {
    return query_one("SELECT provider, api_key, base_url, model, reasoning_effort, is_active, tools_enabled, "
                     "created_at, updated_at "
...
Result<void> LlmConfigRepository::save_provider(const LlmConfig& c) {
    return exec_write(
        ...
        "(provider, api_key, base_url, model, reasoning_effort, is_active, tools_enabled, updated_at) "
        "VALUES (?, ?, ?, ?, ?, ?, ?, datetime('now'))",
        {c.provider, c.api_key, c.base_url, c.model, c.reasoning_effort, c.is_active ? 1 : 0,
         c.tools_enabled ? 1 : 0});
}
